(function(){
    'use.strict';
    let mte_count = 0;//カウント変数
    let mte_repaint_count = 0;//描画数カウント用変数
    let mtePath ='';//SVGデータ保持用変数
//オプションの読み込み
    ...
    //console.log(mte_option);
    mte_option = [{type:'circle', size:'10px', color:'blue', range:50, transition:'none,0,0', opacity:'1.0', render:'default', rotate:'none', translate:'(0,0)'},mte_option];
    ...
    for (mte_count = 0; mte_count < mte_option.range; mte_count++){
        mte_effect += '<effect id="effect'+ mte_count +'" class="effect effect'+ mte_count +'"></effect>';
    }
//type
...
//transition
    mte_option.transition = mte_option.transition.replace(/ /g, ``)
    mte_option.transition = mte_option.transition.split(',');
    //console.log(mte_option.transition[0]);
    for(mte_count = 1;mte_count < mte_option.transition.length; mte_count++){
        mte_option.transition[mte_count] = Number(mte_option.transition[mte_count]);
    }
    //console.log(mte_option.transition);
    switch(mte_option.transition[0]){
        case 'none':mte_option.transition[0] = [mte_option.transition,'',''];break;
        case 'fade':mte_option.transition[0] = [mte_option.transition,'animation: mte_fade '+ mte_option.transition[2] +'s ease-out '+ mte_option.transition[1] +'s 1 normal forwards running;','@keyframes mte_fade{0%{opacity:'+mte_option.opacity+';} 100%{opacity:0.0;}}'];break;
    }
    //console.log(mte_option.transition);
//描画===========================================
...
//静的スタイル
    mte_data.insertAdjacentHTML('afterend',
        ...
        'background:'+mte_option.color[0]+';opacity:'+mte_option.opacity+';display:inline;position:fixed;backface-visibility:hidden; pointer-events: none;-moz-user-select:none;-webkit-user-select:none;-ms-user-select:none;user-select:none;}'+mte_option.transition[0][2]+'</style>'+mtePath);
    let mte_effects = [];
    for(mte_count = 0;mte_count < mte_option.range; mte_count++){
...
//動的スタイル
    ...
    function onMoveEvent(mte_clientX, mte_clientY){
        ...
        if(mte_option.transition[0][0] != 'none'){
            document.getElementById('effect'+ mte_repaint_count).classList.toggle('effect'+ mte_repaint_count);
            document.getElementById('effect'+ mte_repaint_count).offsetWidth = document.getElementById('effect'+ mte_repaint_count).offsetWidth;
            document.getElementById('effect'+ mte_repaint_count).classList.toggle('effect'+ mte_repaint_count);    
        }
        mte_effects[mte_repaint_count].innerHTML = '<style>.effect'+ mte_repaint_count +'{top:'+mte_clientY+'px;left:'+mte_clientX+'px;'+onMoveEvent_count+' transform:rotateZ('+mte_option.rotate[0]+') translateX('+(mte_option.translate[(mte_repaint_count+1)%mte_option.translate.length][0]-50)+'%) translateY('+(mte_option.translate[(mte_repaint_count+1)%mte_option.translate.length][1]-50)+'%);'+
        mte_option.transition[0][1]+'}</style>';
        //console.log(document.getElementById('effect'+ mte_repaint_count)[0]);
        //console.log(mte_effects[mte_repaint_count]);
